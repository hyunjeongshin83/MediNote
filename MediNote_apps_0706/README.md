...
건강 데이터(걸음·심박·수면)는 기기 안(Health Connect · HealthKit)에서 읽어 화면에 보여 줍니다. 사용자가 「읽은 값 클라우드에 저장」을 누르면 **심박만** Supabase measurements(도쿄)로 갑니다. 걸음·수면은 저장 표가 없어 화면에서만 봅니다 (#33 MN-33-1 · CLAUDE.md §3).
...
- `HealthConnectManager` · `HealthKitManager` 는 기기 안(Health Connect / HealthKit)에서 읽어
