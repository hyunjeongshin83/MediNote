...
self.addEventListener("fetch",e=>{
  ...
  // HTML/문서: 네트워크 우선(항상 최신 앱), 오프라인 시 캐시
  if(req.mode==="navigate" || (req.headers.get("accept")||"").indexOf("text/html")>-1){
    e.respondWith(fetch(req).then(r=>{const c=r.clone();caches.open(CACHE).then(x=>x.put(req,c)).catch(()=>{});return r;}).catch(()=>caches.match(req).then(h=>h||caches.match("MediNote_app.html"))));
    return;
  }
  // 그 외 정적 자산: 캐시 우선
